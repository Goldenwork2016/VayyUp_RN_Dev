...
const EmailValidationScreen = ({navigation}) => {
    ...
    const [emailVerificationSent, setEmailVerificationSent] = useStorage('email-verification-sent');
    ...
    return (
        <VUView flex={1} alignItems="center" justifyContent="center" bg={AppStyles.color.bgColor}>
            {verified && <>
                <VUText fontSize={20} color={AppStyles.color.white}>Your email successfully verified.</VUText>
                <Button onPress={handleGotoDashboard}>Goto Dashboard</Button>
            </>}
            {!verified && <>
                <ActivityIndicator size='large' animating={true} color="#fff" />
                <VUText fontSize={20} color={AppStyles.color.white}>Verify Your Email</VUText>
                <VUText px={3} fontSize={16} color={AppStyles.color.white}>Please Check Your email and verify your email address.</VUText>
            </>}
        </VUView>
    );
}
